...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
		...
		String day=(String)request.getAttribute("d");
		String month=(String)request.getAttribute("m");
 ...
 event2=id;
  ...
  <body>
     ...
     <c:forEach items="${activityList}" var="item" varStatus="index">    
                                	 ...
                                	 <a id="at${index.index }"  href="javascript:joinActivity('${path2}activity/joinActivity.action?activityId=${item.activity.activityId }','at${index.index }')"><c:choose><c:when test="${item.joined eq true}">+ Unjoin</c:when> <c:otherwise>+ Join</c:otherwise></c:choose>
                                	    ...
                                	    <c:otherwise>
                                            you are the host
                                              ...
                                              <li><a id="wishList${index.index }"  href="${path2}addWishList/city/${item.activity.activityId}.action">+ Wish List</a></li>
                                              <li><a id="tribusList${index.index }"  href="${path2}addTribusList/city/${item.activity.activityId}.action">+ Tribus List</a></li>
                                          </ul>
                            </div><!--//end .search_info-->
                        </div><!--//end .search_post-->
                      </c:forEach>                  
             ...
             <a href="${path2}activity/searchByDate.action?page=${item}&day=<%=day%>&&month=<%=month%>">
             ${item}
             </a>&nbsp
            </c:forEach>
